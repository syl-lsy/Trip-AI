...
export default (async ({ client, directory }) => {
  ...
  return {
    ...
    "tool.execute.after": async (input) => {
      ...
      if (["bash", "read"].includes(input.tool) && new TextEncoder().encode(outputStr).length > SPILLOVER_LIMIT) {
    ...
    event: async ({ event }) => {
      ...
      try {
        await gcOldFiles(spilloverDir(base), SPILLOVER_TTL_MS, (name: string) => name.endsWith(".txt"))
      } catch {
        // silent
      }
      ...
      try {
        await gcOldFiles(archiveDir(base), ARCHIVE_TTL_MS, (name: string) => name.endsWith(".md") && name !== "INDEX.md")
      } catch {
        // silent
      }
      ...
      try {
        ...
        const hb = raw
          ? JSON.parse(raw)
          : { version: 1, last_maintenance: "", sizes: {}, scheduled: { next_weekly_review: "", next_quarterly_archive: "" }, history: [] }
        ...
        hb.sizes = {
          ...
          daily_notes_count: (await readdir(join(base, "daily"))).filter((f: string) => f.endsWith(".md")).length,
          topic_files_count: (await readdir(join(base, "topics")).catch(() => [] as string[])).filter((f: string) => f.endsWith(".md")).length,
          active_logs_count: (await readdir(join(base, "conversations", "active")).catch(() => [] as string[])).filter((f: string) => f.endsWith(".md")).length,
        }
    ...
    tool: {
      memory_write: tool({
        description: "Write an important fact, decision, or lesson to long-term memory with automatic deduplication",
        args: {
          content: tool.schema.string().describe("What to remember — a concise fact, decision, or lesson"),
        ...
        async execute(args) {
          ...
          for (const l of existing.split("\n")) {
            if (l.includes(args.content.slice(0, 60))) return `Skipped — already exists: ${l.trim()}`
          }
      ...
      memory_search: tool({
        description: "Search across project memory files (MEMORY.md, daily notes, topic files). Use 'semantic' mode for BM25-ranked results with richer context for LLM relevance judgment.",
        args: {
          query: tool.schema.string().describe("Keywords to search for in memory files"),
          max_results: tool.schema.number().optional().default(5).describe("Maximum number of results (default 5)"),
          mode: tool.schema
            .enum(["precise", "semantic"])
            .optional()
            .default("semantic")
            .describe("'precise' = substring match (fast), 'semantic' = BM25 ranking + full metadata (default)"),
        },
        async execute(args) {
          ...
          if (existsSync(dailyDir)) {
            ...
            for (const file of recent) {
              ...
              for (const p of dailyParagraphs) {
                allDocs.push({ text: p.text, file: `daily/${file}`, line: p.startLine, frontmatter: content.slice(0, 500) })
              }
            }
          }
            ...
            for (const file of files.filter((f: string) => f.endsWith(".md"))) {
              ...
              for (const p of topicParagraphs) {
                allDocs.push({
                  ...
                  file: `topics/${file}`,
                  line: p.startLine,
                  frontmatter: fm.description || fm.type ? `desc: "${fm.description}", type: "${fm.type}"` : undefined,
                })
              }
            }
          }
          ...
          if (mode === "precise") {
            ...
            for (let i = 0; i < allDocs.length; i++) {
              ...
              const score = computeBM25(queryTokens, tokens, avgDocLen, totalDocs, docsContainingTerm)
              if (score > 0) {
                results.push({
                  file: doc.file,
                  line: doc.line,
      ...
      session_update: tool({
        description: "Update current session summary (task description, files, decisions, next steps). Survives compaction and resume.",
        args: {
          section: tool.schema
            .enum(["task", "files", "decision", "next"])
            .describe("Section to update: task=任务, files=文件, decision=决策, next=下一步"),
          content: tool.schema.string().describe("Content for the section (plain text or bullet list)"),
        },
        async execute(args) {
      ...
      spillover_list: tool({
        description: "List recent spillover files (large tool outputs saved to disk for reference).",
        args: {
          limit: tool.schema.number().optional().default(10).describe("Max files to list (default 10)"),
          tool_filter: tool.schema.string().optional().describe("Filter by tool name, e.g. 'bash' or 'read'"),
        },
        async execute(args) {
          const spillDir = spilloverDir(base)
      ...
      spillover_read: tool({
        description: "Read the full content of a specific spillover file (use spillover_list to find filenames).",
        args: {
