...
每次 Bug 修复或功能开发完成（经用户验收后），**必须沉淀经验文档**，防止同类问题重复发生。

| 步骤 | 操作                                     | 说明                                                                         |
| ---- | ---------------------------------------- | ---------------------------------------------------------------------------- |
| 1    | 新建 `docs/lessons/<模块>-<问题简述>.md` | 记录问题、根因、修复、教训                                                   |
| 2    | 调用 `memory_write`                      | 记录 `category: "lesson"`，`content` 格式：`<模块>: <根因概要> → <正确做法>` |
...
```
memory_write category="lesson" content="agent.ts: instanceof 在跨 subgraph 消息上失效 → 改用 _getType() 判断消息类型"
```
...
| 文件                                      | 内容                             |
| ----------------------------------------- | -------------------------------- |
| `docs/lessons/phase2-streaming-bugfix.md` | Stream 流式开发中的 7 个踩坑模式 |
